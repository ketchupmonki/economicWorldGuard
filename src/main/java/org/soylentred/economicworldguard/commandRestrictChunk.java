...
public class commandRestrictChunk implements CommandExecutor {
	...
	public commandRestrictChunk(boolean debug, WorldGuardPlugin worldGuard) {
		this.debug = debug;
			...
			for (ProtectedRegion region : regions) {
				if (region.getId().startsWith(player.getName().toLowerCase())){
					if (region.getId().substring(player.getName().length()).equalsIgnoreCase(Integer.toString(chunkLocation[0]) + Integer.toString(chunkLocation[1]))) {
						...
						//Remove Member
						currentMembers.removePlayer(Bukkit.getPlayer(args[0]).getName());
						regionMan.getRegion(region.getId()).setMembers(currentMembers);
						
						player.sendMessage("Restricting chunk from " + args[0] + ".");
						if (debug) {
							Bukkit.getLogger().info("Region " + region.getId() + " restricted from " + Bukkit.getPlayer(args[0]).toString());
						}
						return true;
					}
				}
